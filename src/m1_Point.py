...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.traveled = self.traveled + self.get_distance_from(Point(x, y))

    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
        self.traveled = self.traveled + self.get_distance_from(Point(self.x + dx, self.y + dy))
    ...
    def get_distance_from_start(self):
        ...
        d = self.get_distance_from(start)
        return d
    ...
    def halfway_to(self, p2):
        p2_x = self.x + (p2.x - self.x)/2
        p2_y = self.y + (p2.y - self.y)/2
        point_2 = (p2_x, p2_y)
        return point_2
